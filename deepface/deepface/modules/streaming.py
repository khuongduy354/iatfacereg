    ...
    while True:
        ...
        if freeze is False:
            ...
            if freeze:  
                ...
                (img,demography) = perform_demography_analysis(
        ...
        # check  if freezed  
        if freezed_demo and faces_coordinates and lastTimeFreezed:  
            happyProba = freezed_demo["emotion"]["happy"]   
            (x,y,w,h,le,re,isreal,antd) = faces_coordinates[0] 
            facecoor = {
                "left_eye":le, 
                "right_eye":re
            } 
            passed = handleAction(getAction(),happyProba=happyProba, facialFeatures=facecoor) 
            if passed:     
                passedImg = cv2.imread("./verified.png")
                print("passed ✅") 
            else:  
                print("failed ❌") 
                resetActions()
...
def highlight_facial_areas(
    img: np.ndarray,
    faces_coordinates: List[Tuple[int, int, int, int, Any, Any,bool, float]],
    anti_spoofing: bool = False,
) -> np.ndarray:
    """
    ...
    """
    for x, y, w, h, le, re,is_real, antispoof_score in faces_coordinates:
...
) -> List[Tuple[int, int, int, int, Any,Any,bool, float]]:
    """
...
def extract_facial_areas(
    img: np.ndarray, faces_coordinates: List[Tuple[int, int, int, int, Any, Any,bool, float]]
) -> List[np.ndarray]:
    """
    ...
    """
    detected_faces = []
    for x, y, w, h, left_eye, right_eye,is_real, antispoof_score in faces_coordinates:
...
def perform_facial_recognition(
    ...
    """
    for idx, (x, y, w, h,le,re, is_real, antispoof_score) in enumerate(faces_coordinates):
...
def overlay_emotion(
    ...
    """
    emotion_probas.pop("fear")
    emotion_probas.pop("angry")   
    emotion_probas.pop("neutral")   
    emotion_probas.pop("sad")   
    emotion_probas.pop("disgust")   
    emotion_probas.pop("surprise")   
